...
| 기준 | DFS | BFS |
| --- | --- | --- |
| 자료구조 | 스택(재귀) | 큐(Queue), 덱(Dequeue) |
| 탐색 순서 | 깊이 우선 | 너비(레벨) 우선 |
| 사고 방식 | 한 경로씩 | 상태 집합 |
| 최단 거리 | ▵ | ○ |
| 모든 경우 탐색 | ○ | ▵ |
- BFS에서 필요한 연산: `push`(enqueue), `pop front`(dequeue)
- 파이썬에는 전용 Queue 자료구조가 기본 내장되어 있지 않음
- `list`로 큐처럼 만들 수 있지만, `pop(0)`은 맨 앞을 제거하면 나머지를 전부 앞으로 당겨야 하기 때문에 O(N)
- `collections.deque`을 사용한다면, `append()`/`popleft()`을 통해 양쪽 끝 삽입/삭제가 최적화되어 O(1)